...
0
|| checktplrefresh('./template/fxiaomi/common/header.htm', './template/fxiaomi/common/header_common.htm', 1525338093, '2', './data/template/2_2_common_header_forum_post.tpl.php', './template/fxiaomi', 'common/header_forum_post')
|| checktplrefresh('./template/fxiaomi/common/header.htm', './template/fxiaomi/common/pubsearchform.htm', 1525338093, '2', './data/template/2_2_common_header_forum_post.tpl.php', './template/fxiaomi', 'common/header_forum_post')
;?>
...
<meta name="description" content="RT-Thread社区官方论坛是RT-Thread开发者的大本营，在这里交流rt-thread使用技巧心得，了解rtt最新动态，参与活动互动，为rt-thread开发者们打造一个良好的互动交流平台。">
<meta name="keywords" content="RT-Thread论坛,RT-Thread社区,RT-Thread官方论坛">
...
<div id="user_con_menu" class="p_pop user_con" style="display: none;">
<p>
积分：<?php echo $_G['member']['credits'];?><br>
用户组：<span class="uname"><?php echo $_G['group']['grouptitle'];?></span>
</p>
<div>
...
<?php } } $hasforum = 0;?><!-- //小米公共头部 -->
<div id="editor_bg"></div>  <!-- 编辑器防点击背景 -->
<div class="xmcomm_header_wrap">
<div class="xmcomm_header">
<?php if(!isset($_G['setting']['navlogos'][$mnid])) { ?><a href="<?php if($_G['setting']['domain']['app']['default']) { ?>http://<?php echo $_G['setting']['domain']['app']['default'];?>/<?php } else { ?>./<?php } ?>" title="<?php echo $_G['setting']['bbname'];?>" class="logo"><?php echo $_G['style']['boardlogo'];?> </a><?php } else { ?><?php echo $_G['setting']['navlogos'][$mnid];?><?php } ?>
<div class="header_menu">
<ul><?php if(is_array($_G['setting']['navs'])) foreach($_G['setting']['navs'] as $nav) { if($nav['navname']=='版块') { ?>
<li id="mn_section" onmouseover="showMenu({'ctrlid':this.id,'ctrlclass':'hover','duration':2})"><a href="javascript:;" hidefocus="true">版块 <i class="header_icon"></i></a></li><?php $hasforum = 1;?><?php } elseif($nav['available'] && (!$nav['level'] || ($nav['level'] == 1 && $_G['uid']) || ($nav['level'] == 2 && $_G['adminid'] > 0) || ($nav['level'] == 3 && $_G['adminid'] == 1))) { ?><li <?php if($mnid == $nav['navid']) { ?>class="a" <?php } ?><?php echo $nav['nav'];?>></li><?php } } ?>
</ul>
<?php if(!empty($_G['setting']['pluginhooks']['global_nav_extra'])) echo $_G['setting']['pluginhooks']['global_nav_extra'];?>
</div>
<div id="hscbar_menu1" class="cl2"><?php if($_G['setting']['search']) { $slist = array();?><?php if($_G['fid'] && $_G['forum']['status'] != 3 && $mod != 'group') { ?><?php
$slist[forumfid] = <<<EOF
...
<?php } ?></div><?php include template('common/header_userstatus'); ?><div id="toptb" class="diybtn cl">
<?php echo $diynav;?>
</div>
</div>
</div>
...
<?php } } if($hasforum>0) { require './config/config_global.php';?><?php $prefix=$_config['db']['1']['tablepre']?><?php $forumaa = DB::fetch_all("select fid, fup, name ,displayorder from " .DB::table('forum_forum')." where status=1 ORDER BY displayorder");?><?php $list_index=array();?><?php $lists=array();?><?php if(is_array($forumaa)) foreach($forumaa as $key => $t1) { if($t1['fup']==0) { $list_index[$t1[fid]]=$t1?><?php } else { $lists[$t1[fid]]=$t1?><?php } $forum_array[$t2[catid]]=$t2[catname];?><?php } ?>
<div class=" mn_section_menu" id="mn_section_menu" ctrlid="mn_section" ctrlclass="hover" style="display: none;left: 110px !important;" ><?php if(is_array($list_index)) foreach($list_index as $key => $value) { ?><h4><?php echo $value['name'];?></h4>
<ul><?php if(is_array($lists)) foreach($lists as $key1 => $value1) { if($value1['fup']==$key) { ?>
<li><a href="forum.php?mod=forumdisplay&amp;fid=<?php echo $value1['fid'];?>"><?php echo $value1['name'];?></a>
</li>
<?php } } ?>
</ul>
<?php } ?>
...
<?php } ?>
<?php echo $_G['setting']['menunavs'];?><?php echo adshow("subnavbanner/a_mu");?><?php } ?>
