...
#define CSACTIVE 	GPIO_ResetBits(GPIOD,GPIO_Pin_0)//GPIOD->ODR &= ~GPIO_ODR_ODR_0;
#define CSPASSIVE	GPIO_SetBits(GPIOD,GPIO_Pin_0)//GPIOD->ODR |= GPIO_ODR_ODR_0;
...
unsigned char
SPI3_ReadWrite(unsigned char writedat)
{
	int data;
...
rt_uint8_t
spi_read_op(rt_uint8_t op, rt_uint8_t address)
{
...
//   return dat;
  ...
  SPI_I2S_SendData(SPI3, (op | (address & ADDR_MASK)));
...
// 参数: 命令,地址,数据
void
spi_write_op(rt_uint8_t op, rt_uint8_t address, rt_uint8_t data)
{
  rt_uint32_t level;
...
void
enc28j60_set_bank(rt_uint8_t address)
{
...
void
spi_read_buffer(rt_uint8_t* data, rt_size_t len)
{
  CSACTIVE
  ;

  SPI_I2S_SendData(SPI3, ENC28J60_READ_BUF_MEM);
  while (SPI_I2S_GetFlagStatus(SPI3, SPI_I2S_FLAG_BSY) == SET)
    ;
...
void
spi_write(rt_uint8_t address, rt_uint8_t data)
{
  // set the bank
  enc28j60_set_bank(address);
  // do the write
...
void
enc28j60_phy_write(rt_uint8_t address, rt_uint16_t data)
{
...
// read upper 8 bits
rt_uint16_t
enc28j60_phy_read(rt_uint8_t address)
{
...
void
enc28j60_clkout(rt_uint8_t clk)
{
...
rt_inline void
enc28j60_interrupt_enable(rt_uint32_t level)
{
  /* switch to bank 0 */
  enc28j60_set_bank(EIE);
...
enc28j60(void)
{
  ...
  rt_kprintf("       0x%02x  0x%02x  0x%02x  0x%02x  0x%02x\n", spi_read(ECON1),
      spi_read(ECON2), spi_read(ESTAT), spi_read(EIR), spi_read(EIE));
  rt_kprintf("MAC  : MACON1 MACON3 MACON4\n");
  rt_kprintf("       0x%02x   0x%02x   0x%02x\n", spi_read(MACON1),
      spi_read(MACON3), spi_read(MACON4));
  ...
  rt_kprintf("0x%02x    0x%02x    0x%04x\n", spi_read(ERXFCON),
      spi_read(EPKTCNT), (spi_read(MAMXFLH) << 8) | spi_read(MAMXFLL));
  ...
  rt_kprintf("       0x%04x 0x%04x 0x%02x     0x%02x     0x%02x\n",
      ...
      (spi_read(ETXNDH) << 8) | spi_read(ETXNDL), spi_read(MACLCON1),
      spi_read(MACLCON2), spi_read(MAPHSUP));
}
#include <finsh.h>
...
enc28j60_init(rt_device_t dev)
{
  ...
  spi_write_op(ENC28J60_BIT_FIELD_SET, MACON3,
...
rt_err_t
enc28j60_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
  switch (cmd)
    {
  case NIOCTL_GADDR:
...
rt_err_t
enc28j60_open(rt_device_t dev, rt_uint16_t oflag)
{
  return RT_EOK;
}
...
/* Close the interface */
rt_err_t
enc28j60_close(rt_device_t dev)
{
  return RT_EOK;
}
...
/* Read */
rt_size_t
enc28j60_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
...
/* Write */
rt_size_t
enc28j60_write(rt_device_t dev, rt_off_t pos, const void* buffer,
    rt_size_t size)
{
  rt_set_errno(-RT_ENOSYS);
  return 0;
 ...
 */
rt_err_t
enc28j60_tx(rt_device_t dev, struct pbuf* p)
{
  struct pbuf* q;
  rt_uint32_t len;
  rt_uint8_t* ptr;
  ...
  for (q = p; q != NULL; q = q->next)
    {
      ...
      SPI_I2S_SendData(SPI3, ENC28J60_WRITE_BUF_MEM);
      while (SPI_I2S_GetFlagStatus(SPI3, SPI_I2S_FLAG_BSY) == SET)
        ;
...
//  GPIO_Init(GPIOC, &GPIO_InitStructure);
  ...
  GPIO_Init(GPIOD, &GPIO_InitStructure);                //PD0  CS


  /* configure PD1 as external interrupt */
