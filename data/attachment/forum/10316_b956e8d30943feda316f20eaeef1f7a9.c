...
#define TELNET_TX_BUFFER    256


struct telnet_session *telnet = NULL;
...
/* 向环形buffer中写入数据 */
rt_size_t rb_put(struct rb* rb, const rt_uint8_t *ptr, rt_uint16_t length)
{
    rt_size_t size;
...
/* 向环形buffer中写入一个字符 */
rt_size_t rb_putchar(struct rb* rb, const rt_uint8_t ch)
{
    rt_uint16_t next;
...
/* 从环形buffer中读出数据 */
rt_size_t rb_get(struct rb* rb, rt_uint8_t *ptr, rt_uint16_t length)
{
    rt_size_t size;
...
rt_size_t rb_available(struct rb* rb)
{
    rt_size_t size;
...
void telnet_rx_callback(struct netconn *conn, enum netconn_evt evt, rt_uint16_t len)
{
...
void telnet_send_option(struct telnet_session* telnet, rt_uint8_t option, rt_uint8_t value)
{
    rt_uint8_t optbuf[4];
...
void telnet_thread(void* parameter)
{
    ...
    while (1)
    {
        ...
        newconn->callback = telnet_rx_callback;

        rt_kprintf("new telnet connection, switch finsh to telnet...\n");
...
void telnet_svr(void)
{
	...
	rt_thread_delay(500); //waiting netwotk ready
