...
{   
   ...
   // netifapi_netif_set_link_down(stm32_eth_device->netif);	
	  //netifapi_netif_set_link_up(stm32_eth_device->netif);
...
#   if RMII_TX_GPIO_GROUP == 1
    ...
    GPIO_PinAFConfig(GPIOG, GPIO_PinSource11, GPIO_AF_ETH); /* RMII_TX_EN *///STM32F429BI   为G11
...
#define PHY_MICR_LINK_INT_EN   ((uint16_t)0x0020)
...
//  /* Enable output interrupt events to signal via the INT pin */
//  tmpreg |= (uint16_t)(PHY_MICR_INT_EN | PHY_MICR_INT_EN);
//  if(!(ETH_WritePHYRegister(PHYAddress, PHY_MICR, tmpreg)))
//  {
//    /* Return ERROR in case of write timeout */
//    return ETH_ERROR;
...
{
        int value;
        value = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, PHY_BCR); 
        rt_kprintf("PHY_BCR = 0x%04X\r\n", value);
        value = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, PHY_BSR);
        rt_kprintf("PHY_BSR = 0x%04X\r\n", value);
        value = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, PHY_SR);
        rt_kprintf("PHY_SR = 0x%04X\r\n", value);
        value = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, 0x19); 
        rt_kprintf("PHY_CR = 0x%04X\r\n", value);
        value = ETH_ReadPHYRegister(DP83848_PHY_ADDRESS, 0x17); 
        rt_kprintf("PHY_RBR = 0x%04X\r\n", value);
